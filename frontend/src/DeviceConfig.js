...
export default function DeviceConfig() {
  ...
  const user = stored;
  const loggedUser = stored.username;
      ...
      <div className="buttons-row">
        {isOnline && (
         ...
         <button
           className="device-config-button"
           onClick={() =>
             navigate(`/admin/audit/logs/device/${encodeURIComponent(serial)}`)
           }
         >
           Ver Histórico
         </button>
        ...
        <button className="device-config-button" onClick={() => navigate("/devices", { state: location.state })}>
          Voltar ao Dashboard
        </button>
      </div>
